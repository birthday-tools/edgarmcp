...
### Added
- Opt-in anonymous usage telemetry (off by default; enable with `EDGAR_TELEMETRY=1`).
  Sends only an install id, version, Python/OS, and per-tool call/error counts — never
  request content. Fire-and-forget; never blocks or breaks a tool call. Endpoint
  overridable via `EDGAR_TELEMETRY_URL`.
...
[0.1.2]: https://github.com/birthday-tools/edgarmcp/releases/tag/v0.1.2
