...
from edgarmcp.server import build_tools, wrap_tools
...
class _Rec:
    ...
    def record(self, tool, ok):
        self.events.append((tool, ok))


def test_wrap_tools_records_success_and_error():
    rec = _Rec()
    wrapped = wrap_tools(rec, {"good": lambda x: x * 2, "bad": _raise})
    assert wrapped["good"](3) == 6
    with pytest.raises(ValueError):
        wrapped["bad"]()
    assert rec.events == [("good", True), ("bad", False)]
...
def test_build_context_telemetry_disabled_by_default():
    ctx = make_ctx()
    assert ctx.telemetry.enabled is False
...
def test_wrap_tools_telemetry_failure_never_masks_tool():
    wrapped = wrap_tools(_BoomRec(), {"good": lambda x: x * 2, "bad": _raise})
    assert wrapped["good"](5) == 10            # success value preserved, not the telemetry error
    with pytest.raises(ValueError):            # ORIGINAL ValueError, not RuntimeError
        wrapped["bad"]()
